...
int ifc_reset_connections(const char *ifname, const int reset_mask)
{
    ...
    if (reset_mask & RESET_IPV4_ADDRESSES) {
        ...
        ALOGE("NUKE TCP about %s/%s %s:%s", ifname, inet_ntoa(addr), 
            (result == 0) ? "OK":"FAILED", (result == 0) ? "":strerror(errno));

        if (0 == (reset_mask & RESET_DO_NOT_CLEAR_IP_ADDR)) {
            ifc_set_addr(ifname, 0);
        }
        ifc_close();
    } else {
 ...
 * ADD VLAN
 */
int ifc_add_vlan( const char *if_name, unsigned int vid)
{
    int fd, ret;
    ...
    ret = ioctl(fd, SIOCSIFVLAN, &if_request);
    if (ret < 0) {
        ALOGD("ERROR: trying to add %s to VLAN(#%u). error: %s\n",
                        if_name, vid, strerror(errno));
        close(fd);
        return -2;
    }

	ALOGD("Added %s to VLAN(#%u)\n", if_name, vid);
 ...
 * REMOVE VLAN
 */
int ifc_remove_vlan( const char *if_name)
{
    int fd, ret;
    struct vlan_ioctl_args if_request;
    ...
    ret = ioctl(fd, SIOCSIFVLAN, &if_request);
    if (ret < 0) {
		ALOGD("ERROR: trying to remove VLAN -:%s:- error: %s\n",
                        if_name, strerror(errno));
        close(fd);
        return -2;
    }

    ALOGD("Removed VLAN -:%s:-\n", if_name);
