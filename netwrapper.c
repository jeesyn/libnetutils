...
(const char *client_path, const char *server_path)
{
    ...
    __android_log_print(ANDROID_LOG_INFO, LOCAL_TAG,"%s(ctrl->s = %d---->mode=%x)\n", __FUNCTION__, ctrl->s, buf.st_mode);
...
try_again:
	...
	tries++;
	//chmod(ctrl->local.sun_path, 0x777);
	stat(ctrl->local.sun_path, &buf);
	__android_log_print(ANDROID_LOG_INFO, LOCAL_TAG,"%s: ctrl->local.sun_path: %s--->mode=%x\n", __FUNCTION__, ctrl->local.sun_path, buf.st_mode);
    ...
    do {
        ...
        if ( res < 0 ) {
            ...
            if (0 == strncmp(ack_buf, request_buf,pid_and_reqno_len)) {
                ...
                acked = 1;
                ack_buf[res]='\0';
                if ( pColon = strchr(ack_buf, ':') ) {
                    reply = atoi(pColon+1);
                }

                __android_log_print(ANDROID_LOG_INFO, LOCAL_TAG,"recved VALID ack with reply=%d\n", reply);
            }
            else {
...
int netwrapper_main(const char *server_path)
{
	...
	for (;;) {
		...
		if (res > 0 && FD_ISSET(socket_fd, &rfds)) {
            ...
            int reply_len = cmd - request_buf;
            reply_len += sprintf(request_buf + reply_len, ":%d", handler_ret);
            if (sendto(socket_fd, request_buf, reply_len, 0,
